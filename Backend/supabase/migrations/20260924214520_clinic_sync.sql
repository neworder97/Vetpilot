begin;
create table public.clinic_collections (
...
alter table public.clinic_collections enable row level security;
create policy own_clinic_read on public.clinic_collections for select to authenticated using ((select auth.uid())=user_id);
revoke all on public.clinic_collections from public,anon,authenticated;
grant select on public.clinic_collections to authenticated;
-- Only this compare-and-swap operation writes collections. Ownership comes from JWT, never input.
create function public.save_clinic_collection(expected_version integer,collection_items jsonb)
...
begin
 ...
 if expected_version=0 then
  insert into public.clinic_collections(user_id,version,items) values(owner,1,collection_items) on conflict do nothing returning version into next_version;
 else
  update public.clinic_collections set items=collection_items,version=version+1,updated_at=now() where user_id=owner and version=expected_version returning version into next_version;
 end if;
 if next_version is null then raise sqlstate 'PT409' using message='Cloud revision conflict';end if;
 return next_version;
end $$;
revoke all on function public.save_clinic_collection(integer,jsonb) from public,anon;
grant execute on function public.save_clinic_collection(integer,jsonb) to authenticated;
commit;
