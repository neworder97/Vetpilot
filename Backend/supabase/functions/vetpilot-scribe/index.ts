...
Deno.serve(async request=>{
  ...
  try {
    ...
    if(body.action==='transcribe') {
      ...
      if(bytes.length<16 || new TextDecoder().decode(bytes.slice(4,8))!=='ftyp')throw new RequestError('Expected M4A audio');
      const form=new FormData();form.set('file',new Blob([bytes],{type:'audio/mp4'}),'consultation.m4a');
