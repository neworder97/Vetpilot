...
\i Backend/supabase/migrations/20260925214450_custom_medication_sync.sql
set role authenticated;
set request.jwt.claim.sub='11111111-1111-4111-8111-111111111111';
select public.save_medication_collection(0,'[{"id":"aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa","generic":"Custom test"}]'::jsonb);
do $$ begin
 if (select count(*) from public.medication_collections)<>1 then raise exception 'Medication own read failure';end if;
 begin perform public.save_medication_collection(0,'[]'::jsonb);raise exception 'Missing medication conflict';exception when sqlstate 'PT409' then null;end;
 begin update public.medication_collections set version=9;raise exception 'Direct writes allowed';exception when insufficient_privilege then null;end;
end $$;
set request.jwt.claim.sub='22222222-2222-4222-8222-222222222222';
do $$ begin
 if (select count(*) from public.medication_collections)<>0 then raise exception 'Medication cross-account leak';end if;
 begin perform public.save_medication_collection(1,'[]'::jsonb);raise exception 'Medication cross-account overwrite';exception when sqlstate 'PT409' then null;end;
end $$;
set request.jwt.claim.sub='';
do $$begin
 begin perform public.save_medication_collection(0,'[]'::jsonb);raise exception 'Unauthenticated medication write';exception when insufficient_privilege then null;end;
end $$;
reset role;
