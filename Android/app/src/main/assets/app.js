...
function renderDose(){
 ...
 ${(d?!['mL/kg','drops/eye','inch ribbon/eye'].includes(d.doseBasisRaw):['Liquid','Injection'].includes(m.form))?`<div>${field('concentration','Verified concentration ('+((d?.doseBasisRaw.includes('units')?'units/mL':d?.doseBasisRaw.includes('mEq')?'mEq/mL':'mg/mL'))+')',concentration,'text','inputmode="decimal"')}</div>`:''}
