...
enum ScribePDF {
    static func data(encounter: ScribeEncounter, patientID: UUID, translated: Bool) throws -> Data {
        ...
        return renderer.pdfData { context in
            ...
            text("Visit: \(encounter.createdAt.formatted(date: .abbreviated, time: .shortened))")
    ...
    static func export(encounter: ScribeEncounter, patientID: UUID, translated: Bool = false) throws -> URL {
        ...
        try data(encounter: encounter, patientID: patientID, translated: translated).write(to: url, options: [.atomic, .completeFileProtection])
        return url
    }
}
