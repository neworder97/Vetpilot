...
ditto -c -k --keepParent "$STAGING/Payload" ReleaseArtifacts/VetPilot-0.7.1-Signulous.ipa
python3 - <<'PYTHON'
from pathlib import Path
import hashlib,json,os,plistlib,zipfile
r=Path('ReleaseArtifacts');ipa=r/'VetPilot-0.7.1-Signulous.ipa'
with zipfile.ZipFile(ipa) as z:
    ...
    assert info['CFBundleShortVersionString']=='0.7.1', ('version',info['CFBundleShortVersionString'])
    assert info['CFBundleVersion']=='11', ('build',info['CFBundleVersion'])
...
manifest={'source_commit':os.environ['GITHUB_SHA'],'workflow_run':os.environ['GITHUB_RUN_ID'],
 ...
 'limitations':['Source comparison and regression checks do not establish clinical clearance for every patient, product or medication regimen.','Four ambiguous or unsupported presets remain blocked; source-limited entries require an individualized plan.','My Clinic replaces the X-ray tab. User-created/imported content requires clinical review; sharing sends copies, not live sync.','Labwork contains 42 selected entries (29 IDEXX and 13 MSU), not complete provider directories.','Physical iPhone microphone, call interruptions, on-device speech/translation language assets and clinical performance require device validation.','Email Auth and note storage target the configured Supabase project; confirmation email delivery and physical cross-device login need user verification. Cloud AI and account deletion are not activated.','Nutrition uses the PNA calculator method checked 2026-09-23, not a universal nutrition prescription. Unsupported low BCS and below-floor calorie results require an individualized plan.']}
