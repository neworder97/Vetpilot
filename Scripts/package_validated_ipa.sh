...
ditto -c -k --keepParent "$STAGING/Payload" ReleaseArtifacts/VetPilot-0.8.3-Signulous.ipa
python3 - <<'PYTHON'
from pathlib import Path
import hashlib,json,os,plistlib,zipfile
r=Path('ReleaseArtifacts');ipa=r/'VetPilot-0.8.3-Signulous.ipa'
with zipfile.ZipFile(ipa) as z:
    ...
    assert info['CFBundleShortVersionString']=='0.8.3', ('version',info['CFBundleShortVersionString'])
    assert info['CFBundleVersion']=='16', ('build',info['CFBundleVersion'])
